...
Bootstrap first loads valid V4. Otherwise it validates the newest supported source in strict V3, V2, unversioned, V1 order, preserves its exact bytes, and applies deterministic adapters until V4. Only after current-schema validation and domain reconstruction succeeds may V4 be published. Valid V4 always wins normal startup; explicit Restore deliberately re-runs historical precedence even when V4 is valid.
...
Keep the original localStorage payload until parsing, migration, current-schema validation, and in-memory reconstruction all succeed. Persist the new version atomically only after success. If that write fails, continue the valid migrated in-memory session and leave the previous payload available for a later retry; never replace it with a partial document. A successful migration or Restore clears any older pending retry so stale autosave bytes cannot overwrite the newly published state.
...
Historical validation freezes the derived semantics produced by each supported release. A later balance change may normalize derived enemy health for current combat while preserving encounter identity and the bounded remaining-health ratio, but it must not reject an otherwise authentic save merely because the current health/reward formula differs.
...
The pre-player-relative V3 regression at encounter 2170 is canonical migration evidence: it preserves encounter, coins, upgrade levels, automatic unlock, and remaining-health proportion while rejecting modified armor, reward, health bounds, or identity fields. Multi-slot tests keep V3 ahead of a lower legacy encounter and prove Restore plus delayed-write retry cannot regress the restored V4.
