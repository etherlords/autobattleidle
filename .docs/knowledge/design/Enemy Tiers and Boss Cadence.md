...
Bounded transient effects now derive only from immutable presentation events. Ordinary, armored, and critical hits use distinct geometry; death and positive coin rewards are separate cues; boss transitions are stronger than ordinary transitions; and Golden Bug kill/escape cues retain metallic identity without relying on color alone. The presentation layer caps active effects at 12, expires them by bounded frame lifetimes, and deterministically disposes evicted, expired, and scene-replacement geometry and materials. Reduced-motion keeps the static non-color geometry readable while suppressing growth animation. These effects never enter combat state or persistence.

Independent codec-backed browser QA at 1280x800 and 390x844 proved armor, critical, one-hit boss death/spawn, Golden Bug kill/escape, reduced-motion readability, a 30-second bounded session, one canvas, stable resource count, clean console, and isolated context/browser cleanup. Focused tests prove event mapping, cap/eviction, expiry, reduced-motion scale, and exactly-once disposal.
