...
describe("cue buffer map", () => {
  ...
  it("maps manual critical to heavy metal and manual armor to tin", () => {
    expect(
    ...
    ).toBe("impactMetal_heavy_000");
    expect(
      cueFromBattleVisualCue({ kind: "armor", packets: { count: 1, units: 1 }, source: "manual" }),
    ).toBe("impactTin_medium_000");
  });

  it("maps automatic batch hits to soft medium with alternation", () => {
    const cue: BattleVisualCue = {
      kind: "hit",
      packets: { count: 1, units: 1 },
      source: "automatic",
    };
    expect(cueFromBattleVisualCue(cue, 0)).toBe("impactSoft_medium_000");
    expect(cueFromBattleVisualCue(cue, 1)).toBe("impactSoft_medium_001");
  });

  it("maps automatic criticals to light metal with alternation", () => {
    expect(
      cueFromBattleVisualCue(
        { kind: "critical", packets: { count: 1, units: 1 }, source: "automatic" },
        0,
      ),
    ).toBe("impactMetal_light_000");
    expect(
      cueFromBattleVisualCue(
        { kind: "critical", packets: { count: 1, units: 1 }, source: "automatic" },
        1,
      ),
    ).toBe("impactMetal_light_001");
  });

  it("maps scene cues", () => {
    expect(cueFromBattleVisualCue("death")).toBe("impactPlate_medium_000");
    expect(cueFromBattleVisualCue("coin")).toBe("confirmation_001");
    expect(cueFromBattleVisualCue("boss")).toBe("impactBell_heavy_000");
    expect(cueFromBattleVisualCue("golden-kill")).toBe("impactMining_000");
